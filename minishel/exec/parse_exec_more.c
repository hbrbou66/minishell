...
/*   Updated: 2025/08/19 23:29:16 by hbou-dou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../minishell.h"

int	handle_red_in(t_token **lst, t_exec *node)
{
	int (tmp), (a);
	a = check_ambiguous(lst, node);
	if (a != 0)
		return (a);
...
int	handle_red_out(t_token **lst, t_exec *node)
{
	if (node->fd_out != 1)
		close(node->fd_out);
	if ((*lst)->next->ambg)
