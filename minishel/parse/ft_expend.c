...
/*   Updated: 2025/08/20 17:34:25 by hbou-dou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
size_t	ft_len_no_q(t_expand_ctx *ctx, char *val)
{
	size_t (count) = 0;
	size_t (index) = 0;
	bool (double_q) = ctx->expand_state.double_q;
	bool (single_q) = ctx->expand_state.single_q;
	if (!val)
		return (0);
	while (val[index])
	{
...
char	*ft_remove_quotes(t_expand_ctx *ctx, char *tmp)
{
	...
	while (tmp[tmp_i])
	{
		if (tmp[tmp_i] == '\'' && !ctx->expand_state.double_q)
		{
			ctx->expand_state.single_q = !ctx->expand_state.single_q;
			tmp_i++;
		}
		else if (tmp[tmp_i] == '\"' && !ctx->expand_state.single_q)
			(1) && (ctx->expand_state.double_q = !ctx->expand_state.double_q,
				tmp_i++);
		else if (tmp[tmp_i] == '$' && !ctx->expand_state.double_q
			&& !ctx->expand_state.single_q && ft_strchr("\'\"", tmp[tmp_i + 1]))
			tmp_i++;
		else
			(1) && (clean[index] = tmp[tmp_i], tmp_i++, index++);
	}
	clean[index] = '\0';
	return (clean);
}
